sendNotification = function (subject, title, message, user) {
  ...
  var firstName = user.profile &&
    user.profile.firstName ? user.profile.firstName : null;
...
sendSlackNotification = function (type, message, data) {
  ...
  var shouts = Meteor.settings.notifications.slack.shouts[type] ?
    Meteor.settings.notifications.slack.shouts[type] : [];
