...
Template.test.events({
  'click #test-run-button': function onTestRun (event) {
    event.preventDefault();
    Meteor.call("runTest", this.test._id, function onRunTest (err) {
      if (err) {
        console.log(err);
      }
    });
...
Template.test.events({
  'click #test-remove-button': function onItemRemove (event) {
    event.preventDefault();
